...
const tStart = code.indexOf('function TokenizationVisual()');
const endSoftmaxMarker = Buffer.from('SoftmaxVisual').toString('utf8');
const sStart = code.lastIndexOf('function SoftmaxVisual()');

let openBraces = 0;
let sEndIndex = sStart + 'function SoftmaxVisual() {'.length;
let foundInitialBrace = false;
for (; sEndIndex < code.length; sEndIndex++) {
    if (code[sEndIndex] === '{') {
        openBraces++;
        foundInitialBrace = true;
    } else if (code[sEndIndex] === '}') {
        openBraces--;
...
const before = code.substring(0, tStart);
const after = code.substring(sEndIndex);

const newVisuals = `function TokenizationVisual() {
...
fs.writeFileSync('src/App.tsx', before + newVisuals + after);
