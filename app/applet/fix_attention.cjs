...
const expertAttentionCode = \`function AttentionVisual() {
  ...
  return (
    <div className="relative h-[28rem] bg-slate-100/80 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-3xl p-4 flex items-center justify-center overflow-hidden">
      
      <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8 w-full justify-center relative z-10 w-full max-w-5xl px-4">
        {/* Input */}
          ...
          {perspective === 'expert' ? (
            <div className="absolute inset-0 flex flex-col items-center justify-start p-4">
              <span className="text-xl md:text-2xl text-slate-800 dark:text-slate-200 font-serif border-b border-slate-400 pb-2 w-full text-center tracking-wider mb-6 flex items-center justify-center whitespace-nowrap">
                ...
                <span className="text-blue-600 dark:text-blue-400 font-bold ml-2">Softmax</span>
              ...
              <div className="flex items-center justify-center gap-2 md:gap-6 text-xs md:text-sm font-mono text-slate-500 dark:text-slate-400 opacity-90 mt-2 w-full">
                 <div className="flex flex-col gap-1 items-center">
                    ...
                    <div className="grid grid-cols-4 grid-rows-4 gap-[2px]">
                       {[...Array(16)].map((_, i) => <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1+Math.random(), repeat: Infinity }} key={i} className={\\\`w-3 h-3 md:w-4 md:h-4 rounded-sm \\\${i%5===0 ? 'bg-blue-500' : 'bg-blue-300 dark:bg-blue-900'}\\\`}></motion.div>)}
                    </div>
                 </div>
                 <span className="text-xl text-slate-400 font-thin">÷√d_k <span className="text-pink-400 font-bold block text-[10px] uppercase text-center mt-1">Scale</span></span>
                 <div className="flex flex-col gap-1 items-center">
                    <span className="text-pink-400 font-bold tracking-tight">Mask</span>
                    <div className="grid grid-cols-4 grid-rows-4 gap-[2px]">
                       {[...Array(16)].map((_, i) => <div key={i} className={\\\`w-3 h-3 md:w-4 md:h-4 rounded-sm \\\${(i%4) <= Math.floor(i/4) ? 'bg-pink-500/80' : 'bg-slate-200 dark:bg-slate-800'}\\\`}></div>)}
                    </div>
                 </div>
                 <span className="text-xl text-green-500 font-thin">× V <span className="text-green-400 font-bold block text-[10px] uppercase text-center mt-1">Weight</span></span>
                 <div className="flex flex-col gap-1 items-center">
                    <span className="text-green-500 font-bold tracking-tight">Z</span>
                    <div className="grid grid-cols-4 grid-rows-4 gap-[2px]">
                       {[...Array(16)].map((_, i) => <div key={i} className={\\\`w-3 h-3 md:w-4 md:h-4 rounded-sm \\\${i%2===0 ? 'bg-green-400/80' : 'bg-green-700'}\\\`}></div>)}
                    </div>
                 </div>
              </div>
            </div>
...
const startIndex = code.indexOf('function AttentionVisual()');
const endIndex = code.indexOf('function ResidualNormVisual()');
if(startIndex !== -1 && endIndex !== -1) {
  code = code.substring(0, startIndex) + expertAttentionCode + code.substring(endIndex);
