...
function FFNVisual() {
  ...
  return (
    <div className="relative h-96 bg-slate-100/80 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-lg flex flex-col items-center">
        ...
        <div className="flex gap-2">
           {[...Array(6)].map((_, i) => (
             <motion.div key={i} animate={{ y: [0, -3, 0] }} transition={{ delay: i*0.1, duration: 2, repeat: Infinity }} className="w-8 h-8 rounded border border-slate-400 dark:border-slate-600 bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-[10px] font-mono text-slate-500 shadow-inner">
               x_{i}
             </motion.div>
           ))}
        </div>
...
replaceCode(/"6\. 混合专家 \(MoE\)"/, '"7. 混合专家 (MoE)"');
replaceCode(/"7\. 回归与生成 \(Softmax\)"/, '"8. 回归与生成 (Softmax)"');
replaceCode(/"8\. 推理内存墙 \(KV Cache\)"/, '"9. 推理内存墙 (KV Cache)"');

replaceCode(/"6\. Mixture of Experts \(MoE\)"/, '"7. Mixture of Experts (MoE)"');
replaceCode(/"7\. Softmax \& Generation"/, '"8. Softmax & Generation"');
replaceCode(/"8\. Memory Wall \(KV Cache\)"/, '"9. Memory Wall (KV Cache)"');

replaceCode(/"6\. 稀疏混合专家与 SwiGLU \(Sparse MoE\)"/, '"7. 稀疏混合专家 (Sparse MoE)"');
replaceCode(/变体门控线性单元 SwiGLU \\(Swish\\(xW\\) ⊙ xV\\) 以提升信号传播与表征容量。采用 Top-2\\/K/g, '采用 Top-2/K');
replaceCode(/摒弃传统 ReLU，采用变体门控线性单元 SwiGLU \(Swish\(xW\) ⊙ xV\) 以提升信号传播与表征容量。采用 Top-2\/K/g, '采用 Top-2/K');
replaceCode(/核心的激活层 SwiGLU 则是 Shazeer 在 2020 年提出，淘汰了经典的经典 ReLU\/GELU 门控。/g, '');

replaceCode(/"7\. Logits 与 解码采样 \(Decoding\)"/, '"8. Logits 与 解码采样 (Decoding)"');
replaceCode(/"8\. 显存管理机制 \(PagedAttention\)"/, '"9. 显存管理机制 (PagedAttention)"');
...
// <Section className="bg-slate-50/50 ... MoE ...
