...
function GeekPerspectiveView() {
  ...
  const Node = ({ id, w = "w-64", h = "h-16" }: { id: keyof typeof blockData, w?: string, h?: string }) => {
  ...
  return (
    <div className="w-full relative z-10 py-16 px-4 flex flex-col items-center overflow-x-auto min-h-[1200px]">
      <h2 className="text-2xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-600 mb-2">
         {lang === 'zh' ? 'Transformer 流水线极客全景' : 'Transformer Pipeline Geek Map'}
      </h2>
      ...
      <div className="relative flex flex-col items-center gap-6 min-w-[800px]">
         ...
         <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-50" style={{ zIndex: 0 }}>
             ...
             {/* Skip connection 1 */}
             <path d="M 400 220 C 150 250, 150 500, 400 530" stroke="currentColor" className="text-slate-500 dark:text-slate-600" strokeWidth="4" fill="none" />
             <path d="M 400 580 C 150 610, 150 780, 400 810" stroke="currentColor" className="text-slate-500 dark:text-slate-600" strokeWidth="4" fill="none" />
         ...
         <div className="text-slate-500 font-mono mb-4 text-xl">Inputs (Tokens)</div>
         ...
         <div className="text-slate-500 font-mono my-4 text-2xl">+</div>

         <div className="border border-cyan-500/30 bg-slate-100/50 dark:bg-slate-800/50 p-8 rounded-3xl flex flex-col items-center gap-6 relative shadow-[0_0_50px_rgba(6,182,212,0.1)]">
            <div className="absolute -left-32 top-1/2 -translate-y-1/2 text-cyan-600/50 font-mono rotate-180 tracking-widest uppercase text-2xl font-bold" style={{ writingMode: 'vertical-rl' }}>
               Nx Layers
            </div>
         ...
         <div className="text-slate-500 font-mono mt-4 text-xl">Outputs (Next Token)</div>
...
app = app.replace('import { GeekPerspectiveView } from "./components/GeekPerspectiveView";\n', '');
app = app.replace('import { GeekPerspectiveView } from "./components/GeekPerspectiveView";', '');

// Insert it somewhere near the other visual components
app = app.replace(/(function AppContent\(\) \{)/, geekComponent + '\n$1');
